...
# Configuration files
export SYS_ROSBAG_NAME=stlucia_2007.bag

input=(
  'RatSlam' 'waitForRos; roslaunch $ROS_LAUNCH_PATH/ratslam_stlucia2007.launch
'
)

input+=(
  'PlayDataset' 'waitForRos; rosparam set /use_sim_time true &&
          rosbag play --pause --clock $ROS_BAG_PATH/$SYS_ROSBAG_NAME /stlucia/camera/image/compressed:=/camera/image/compressed
'
